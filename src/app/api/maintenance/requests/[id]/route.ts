...
async function proxyDetail(request: Request, { params }: Context, init: RequestInit = {}) {
  return maintenanceProxy<MaintenanceRequest>(request, detailPath(apiEndpoints.maintenance, (await params).id), init);
}
