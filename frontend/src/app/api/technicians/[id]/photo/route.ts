...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const response = await fetch(`${baseUrl.replace(/\/+$/, "")}${actionPath(apiEndpoints.technicians, (await params).id, "photo")}`, {
    cache: "no-store", headers: { Authorization: `Bearer ${access}` },
  });
  if (!response.ok) return Response.json({ detail: "Profile photo could not be loaded." }, { status: response.status });
